...
esp_err_t initializeApplication(MainTaskState *state, MainTaskResources *res) {
  ...
  if (res == NULL) return ESP_ERR_INVALID_ARG;
  ...
  if (errRes.errMutex == NULL) return ESP_ERR_NO_MEM;
  res->errRes = &errRes;
  ...
  tls = esp_tls_init();
  if (tls == NULL) return ESP_ERR_NO_MEM;

  /* initialize http client */
  res->client = initHttpClient();
  if (res->client == NULL) return ESP_FAIL;

  /* create tasks */
  err = createOTATask(&otaTask, res->errRes);
  if (err != ESP_OK) return err;
  if (otaTask == NULL) return ESP_FAIL;
