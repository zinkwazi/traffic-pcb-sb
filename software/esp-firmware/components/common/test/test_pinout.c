 ...
 * test_pinout.c
 * 
 * This file contains unit tests which verify the pinout.h file with the help
 * of human verification. These tests require that the toggle button and OTA button
 * be correct in the pinout to allow the verifier to interact with the tests.
 */
...
esp_err_t initializeVerificationButtons(VerificationResources *resources) {
    ...
    TEST_ASSERT_EQUAL(ESP_OK, gpio_set_direction(T_SW_PIN, GPIO_MODE_INPUT));
    TEST_ASSERT_EQUAL(ESP_OK, gpio_set_intr_type(T_SW_PIN, GPIO_INTR_NEGEDGE));
    TEST_ASSERT_EQUAL(ESP_OK, gpio_isr_handler_add(T_SW_PIN, OTAButtonISR, &buttonParams));
    return ESP_OK;
}
...
void OTAButtonISR(void *params) {
    ...
    struct timeval currTime;
    SemaphoreHandle_t sema = ((*VerificationResources) params)->sema;
    bool *correct = ((*VerificationResources) params)->sema;
    bool *waiting = ((*VerificationResources) params)->waiting;
...
void assertHumanVerifies(char *message, VerificationResources res) {
    ESP_LOGI(TAG, "%s", message);
    *(res.waiting) = true;
    while (xSemaphoreTake(res.sema, INT_MAX) != pdTRUE) {}
    TEST_ASSERT_NOT_EQUAL(true, *(res.waiting));
    TEST_ASSERT_EQUAL(true, *(res.correct));
}

TEST_CASE("status_indicators", "[pinout]")
{
    VerificationResources res;
    TEST_ASSERT_EQUAL(ESP_OK, initializeVerificationButtons(&res));
    assertHumanVerifies("Verify Toggle Button? Y/N", &res);
}