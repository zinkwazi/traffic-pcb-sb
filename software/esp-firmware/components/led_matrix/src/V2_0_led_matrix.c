...
static SemaphoreHandle_t mat4Mutex;
...
static mat_err_t matGetRegisters(uint8_t *result1, uint8_t *result2, uint8_t *result3, uint8_t *result4, uint8_t page, uint8_t addr);
...
static mat_err_t matSetRegistersSeparate(uint8_t page, uint8_t addr, uint8_t mat1val, uint8_t mat2val, uint8_t mat3val, uint8_t mat4val);
...
mat_err_t matInitializeBus1(i2c_port_num_t port, gpio_num_t sdaPin, gpio_num_t sclPin)
{
    ...
    mat_err = (mat_err_t) i2c_new_master_bus(&master_bus_config, &sI2CBus1);
    if (mat_err == ESP_ERR_INVALID_ARG) THROW_MAT_ERR((mat_err_t) ESP_FAIL);
    if (mat_err != ESP_OK) return mat_err;

    mat_err = (mat_err_t) i2c_master_bus_add_device(sI2CBus1, &matrix_config, &sMat1Handle);
    if (mat_err == ESP_ERR_INVALID_ARG) THROW_MAT_ERR((mat_err_t) ESP_FAIL);
    if (mat_err != ESP_OK) return mat_err;

    matrix_config.device_address = MAT2_ADDR;
    mat_err = (mat_err_t) i2c_master_bus_add_device(sI2CBus1, &matrix_config, &sMat2Handle);
    if (mat_err == ESP_ERR_INVALID_ARG) THROW_MAT_ERR((mat_err_t) ESP_FAIL);
    if (mat_err != ESP_OK) return mat_err;
    ...
    if (mat_err == ESP_ERR_INVALID_ARG) THROW_MAT_ERR((mat_err_t) ESP_FAIL);
    if (mat_err == ESP_ERR_INVALID_STATE) THROW_MAT_ERR((mat_err_t) ESP_FAIL);
 ...
 * 
 * @returns ESP_OK if successful.
 * ESP_ERR_NOT_FOUND if a matrix on the second I2C bus could not be found.
 * MAT_ERR_MUTEX_TIMEOUT if taking a matrix mutex timed out.
 * MAT_ERR_MUTEX if releasing a matrix mutex failed.
 * ESP_FAIL if an unexpected error occurred.
 */
mat_err_t matInitializeBus2(i2c_port_num_t port, gpio_num_t sdaPin, gpio_num_t sclPin)
{
    ...
    /* Initialize I2C bus 1 */
    mat_err = (mat_err_t) i2c_new_master_bus(&master_bus_config, &sI2CBus2);
    if (mat_err == ESP_ERR_INVALID_ARG) THROW_MAT_ERR((mat_err_t) ESP_FAIL);
    if (mat_err != ESP_OK) return mat_err;

    mat_err = (mat_err_t) i2c_master_bus_add_device(sI2CBus2, &matrix_config, &sMat3Handle);
    if (mat_err == ESP_ERR_INVALID_ARG) THROW_MAT_ERR((mat_err_t) ESP_FAIL);
    if (mat_err != ESP_OK) return mat_err;

    matrix_config.device_address = MAT4_ADDR;
    mat_err = (mat_err_t) i2c_master_bus_add_device(sI2CBus2, &matrix_config, &sMat4Handle);
    if (mat_err == ESP_ERR_INVALID_ARG) THROW_MAT_ERR((mat_err_t) ESP_FAIL);
    if (mat_err != ESP_OK) return mat_err;
    ...
    mat4Mutex = xSemaphoreCreateMutex();
    if (mat4Mutex == NULL) THROW_MAT_ERR((mat_err_t) ESP_FAIL);
    ...
    mat_err = matSetPage(sMat4Handle, CONFIG_PAGE);
    if (mat_err != ESP_OK) mat_err = handleMatSetPageErr(mat_err, sMat4Handle);
    if (mat_err == ESP_ERR_INVALID_ARG) THROW_MAT_ERR((mat_err_t) ESP_FAIL);
    if (mat_err == ESP_ERR_INVALID_STATE) THROW_MAT_ERR((mat_err_t) ESP_FAIL);
    if (mat_err == ESP_ERR_TIMEOUT) return (mat_err_t) ESP_ERR_NOT_FOUND;
    if (mat_err == ESP_ERR_INVALID_RESPONSE) return (mat_err_t) ESP_ERR_NOT_FOUND;
    if (mat_err != ESP_OK) return mat_err;
    mat_err = giveMatrixMutex(sMat4Handle); // handleMatSetPageErr does not release if ESP_OK
    if (mat_err != ESP_OK) return MAT_ERR_MUTEX;
...
static mat_err_t matSetConfig(uint8_t bitMask, uint8_t setting)
{
    ...
    mat_err = matGetRegisters(&mat1Cfg, &mat2Cfg, &mat3Cfg, &mat4Cfg, CONFIG_PAGE, CONFIG_REG_ADDR);
    if (mat_err == ESP_ERR_INVALID_ARG) THROW_MAT_ERR((mat_err_t) ESP_FAIL);
    if (mat_err != ESP_OK) return mat_err;
    ...
    mat_err = matSetRegistersSeparate(CONFIG_PAGE, CONFIG_REG_ADDR, mat1Cfg, mat2Cfg, mat3Cfg, mat4Cfg);
    if (mat_err == ESP_ERR_INVALID_ARG) THROW_MAT_ERR((mat_err_t) ESP_FAIL);
    if (mat_err != ESP_OK) return mat_err;
...
static mat_err_t matGetRegisters(uint8_t *result1, uint8_t *result2, uint8_t *result3, uint8_t *result4, uint8_t page, uint8_t addr)
{
    ...
    if (result4 != NULL)
    {
        mat_err = matGetRegister(&localRes4, sMat4Handle, page, addr);
        if (mat_err == ESP_ERR_INVALID_ARG) THROW_MAT_ERR((mat_err_t) ESP_FAIL);
        if (mat_err != ESP_OK) return mat_err;
    }
...
static mat_err_t matSetRegisters(uint8_t page, uint8_t addr, uint8_t data)
{
    ...
    if (mat_err != ESP_OK) return mat_err;
    mat_err = matSetRegister(sMat4Handle, page, addr, data);
    if (mat_err != ESP_OK) return mat_err;
...
static mat_err_t matSetRegistersSeparate(uint8_t page, uint8_t addr, uint8_t mat1val, uint8_t mat2val, uint8_t mat3val, uint8_t mat4val)
{
    ...
    if (mat_err != ESP_OK) return mat_err;
    mat_err = matSetRegister(sMat4Handle, page, addr, mat4val);
    if (mat_err != ESP_OK) return mat_err;
